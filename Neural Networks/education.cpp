...
int main(int argc, char *argv[])
{
    ...
    while (getline(trainFile, line))    //ignore the first line
    {
        stringstream ss(line);
        string str;
        vector<double> lineArray;
        while (getline(ss, str, ','))
            lineArray.push_back(atof(str.c_str())/100);
    ...
    while (getline(trainKeyFile, line))
    {
        ...
        string str;
        while (getline(ss, str, ','))
            trainKey.push_back(atof(str.c_str())/100);
    }
    //get normalized dev data
    getline(devFile, line);
    while (getline(devFile, line))    //ignore the first line
    {
        stringstream ss(line);
        string str;
        vector<double> lineArray;
    ...
    LR = 0.005;
    double inhd_weight_update[6][3] = {0};
    ...
    for(i = 0; i < 100; i++)
    {
        ...
        for(j = 0; j < trainNum; j++)
        {
            ...
            for (int k=0;k<3;k++)
            {
                ...
                inhd_weight_update[5][k] += LR*delta_weight1[k]*trainData[j][4];
            }
        }
        ...
        for (int k=0;k<3;k++)
        {
            ...
            inhd_weight[5][k] += inhd_weight_update[5][k];
        }
    ...
    for(i = 0; i < 50; i++)
    {
        ...
        for(j = 0; j < trainNum; j++)
        {
            ...
            for (int k=0;k<3;k++)
            {
                ...
                inhd_weight[5][k] += LR*delta_weight1[k]*trainData[j][4];
            }
            loss += 0.5*(trainKey[j]-result)*(trainKey[j]-result);
        }

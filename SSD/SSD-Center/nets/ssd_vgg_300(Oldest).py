...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        with tf.name_scope('center_loss'):
            loss, centers_update_op = get_center_loss(logits, gclasses)
            loss = tf.div(tf.reduce_sum(loss * fpmask), batch_size, name='value')
            tf.losses.add_loss(loss)
...
def get_center_loss(logits, gclasses, alpha=0.5, num_classes=21):

    len_logits = logits.get_shape()[1]
    centers = tf.get_variable('centers', [num_classes, len_logits], dtype=tf.float32,
                              initializer=tf.constant_initializer(0), trainable=False)
    labels = tf.reshape(gclasses, [-1])

    centers_batch = tf.gather(centers, labels)
    loss = tf.nn.l2_loss(logits - centers_batch)

    diff = centers_batch - logits

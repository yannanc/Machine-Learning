    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        for i, layer in enumerate(feat_layers):
            with tf.variable_scope(layer + '_loss'):
                ...
                feature_mean = tf.expand_dims(tf.reduce_mean(layer_features, axis=0), 0)
                ini_feat = tf.tile(feature_mean, [num_classes, 1])

                loss, center_op = get_center_loss(layer_features, layer, labels_one_layer, ini_feat)
                loss = tf.div(tf.reduce_sum(0.01 * loss * fpmask), n_positives * batch_size, name='value')
...
def get_center_loss(layer_feature, layer, labels_one_layer, ini_feat, alpha=0.5, num_classes=21):
    ...
    centers = tf.get_variable('centers_'+layer, [num_classes, feature_shape], dtype=tf.float32,
    ...
    unique_label, unique_idx, unique_count = tf.unique_with_counts(labels_one_layer)
